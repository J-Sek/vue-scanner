...
const otherDependencies = chartkick + 'draggable,quill-editor,vue-cropper,v-chart,tip-tap,vue-plyr'.split(',');
...
const ignoreMetaElements = 'component,scrollbar,template,slot,keep-alive,n-link,nuxt,nuxt-child,client-only'.split(',');
...
export async function analyzeFile(baseDir: string, path: string, name: string): Promise<ScanItem> {
  ...
  const templateWithoutComments = template
    .replace(/<!--([^]*?)-->/mg, '')

  const vDeps = [...(templateWithoutComments.match(/<[a-z][a-z\-0-9]+(\.|\r|\n| |:|>)/ig) ?? [])]
    ...
    .map(toKebabCase)
    .sort();
  const vuetifyDirectives = [...(templateWithoutComments.match(/(\n|\n\s+|\()[a-z\-]+(\.|=|\n| |\))/ig) ?? [])]
  ...
  const fileName = name.split('/').at(-1)!.replace('.vue', '')
  return {
    path,
    fileName: fileName,
